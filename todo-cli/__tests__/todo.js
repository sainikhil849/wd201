...
describe('TodoList Test Suite', () => {
  ...
  test('Should not retrieve completed overdue items', () => {
    todos.add({ title: 'Completed Overdue Todo', dueDate: yesterday.toISOString().slice(0, 10), completed: true });
    const overdueCount = todos.overdue().length;
    expect(todos.overdue().length).toBe(overdueCount);
  });

  test('Should not retrieve completed due today items', () => {
    todos.add({ title: 'Completed Due Today Todo', dueDate: today.toISOString().slice(0, 10), completed: true });
    const dueTodayCount = todos.dueToday().length;
